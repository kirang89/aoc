...
let filter_unordered rules updates =
  List.filter
    (fun u ->
      u
...
let order_update rules update =
  let page_nos = update |> String.split_on_char ',' |> List.map int_of_string in
  page_nos
  |> List.sort (fun p1 p2 ->
         match is_after p1 p2 rules with true -> -1 | false -> 1)
...
let run () =
  ...
  let unordered_updates = filter_unordered parsed_rules updates in
  Utils.print_list (fun i -> i) unordered_updates;
  let ordered_updates =
    List.map (fun u -> order_update parsed_rules u) unordered_updates
  in
  let sum =
    List.fold_left
      (fun acc u ->
